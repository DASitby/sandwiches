...
const getSandwich = (listingID,userID) => {
  return db.query(
    `SELECT title, description, size, gluten_free, vegetarian, protein, bread_type, price, admins.name as seller_name, admins.email as seller_email, admins.phone_number as seller_phone_number, thumbnail_url, (SELECT CASE WHEN EXISTS
      (SELECT id
      FROM favorites
